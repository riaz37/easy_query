...
# Report Task Manager for Background Processing
class ReportTaskManager:
    """Database manager for report background tasks"""
    ...
    def init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            try:
                # Try to add columns that might not exist in older versions
                columns_to_add = [
                    ('total_queries', 'INTEGER DEFAULT 0'),
                    ('processed_queries', 'INTEGER DEFAULT 0'),
                    ('successful_queries', 'INTEGER DEFAULT 0'),
                ...
                for column_name, column_type in columns_to_add:
                    try:
                        conn.execute(f'ALTER TABLE report_tasks ADD COLUMN {column_name} {column_type}')
                ...
                # Continue anyway, the table should still work
        ...
        with self.get_connection() as conn:
            conn.execute('''
                INSERT INTO report_tasks (
                    ...
                    progress, current_step, total_queries, processed_queries, successful_queries,
...
#     """
#     Generate a complete report for a user with optional query filtering.
    
#     This endpoint:
#     1. Takes a user_id and optional user_query
#     2. Automatically fetches the user's current database
#     3. Generates comprehensive reports with graphs as PNG images
#     4. Returns all data directly in JSON format
    
#     Args:
#         request: ReportGenerationRequest containing user_id and optional user_query
        
#     Returns:
#         ReportGenerationResponse with complete report data and metadata
#     """
#     try:
#         # Use the async version for better performance
#         result = await generate_complete_report_async(
#             user_id=request.user_id,
#             user_query=request.user_query
#         )
        
#         # Ensure database_id is properly formatted
#         if 'database_id' in result and result['database_id'] is not None:
#             result['database_id'] = str(result['database_id'])
...
# @router.post("/generate-report-sync", response_model=ReportGenerationResponse)
# def generate_report_sync_api(request: ReportGenerationRequest):
#     """
#     Synchronous version of report generation endpoint.
    
#     This endpoint uses the synchronous version of generate_complete_report
#     for cases where async processing is not preferred.
#     """
#     try:
...
async def run_background_report_generation(task_id: str, user_id: str, user_query: Optional[str] = None):
    """
    Background task function that runs report generation with comprehensive tracking.
...
async def generate_report_background(
    ...
    try:
        ...
        background_tasks.add_task(
            run_background_report_generation,
            task_id,
            request.user_id,
            request.user_query
        )
        
        print(f"✅ Background task {task_id} created and queued")
