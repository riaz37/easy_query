...
Orchestrates document saving and embedding generation in sequence.
"""
...
from app.unstructured.agent.data_storage import data_save
from app.unstructured.agent.embedding import embed_gen
from app.unstructured.agent.sub_intent import sub_intent_gen, SubIntentConfig, SubIntentManager, DatabaseConfig
...
class DocumentPipelineOrchestrator:
    """Orchestrates the complete document processing and embedding pipeline."""
...
def run_individual_stage(
    ...
    Args:
        stage: Stage to run - 'store', 'embed', or 'sub_intent'
    ...
    try:
        if stage.lower() == 'store':
            ...
            raise ValueError(f"Invalid stage: '{stage}'. Must be 'store', 'embed', or 'sub_intent'")
...
def run_partial_pipeline(
    ...
    **kwargs
    ...
    Args:
        stages: List of stages to run e.g., ['store', 'embed'] or ['embed', 'sub_intent']
    ...
    try:
        # Validate stages
        valid_stages = ['store', 'embed', 'sub_intent']
        for stage in stages:
...
if __name__ == "__main__":
    ...
    # # Example 4: Run specific combination of stages
    # partial_result = run_partial_pipeline(
    #     stages=['embed', 'sub_intent'],
    #     batch_size=20,
    #     similarity_threshold=0.75
    # )
    # print(f"Partial pipeline: {'✅' if partial_result['success'] else '❌'}")
    
    # # Example 5: Standalone sub-intent generation (async)
