...
from data_sources.file_data.app.semi_structured.data_upload_v2 import process_file_pipeline
...
class EnhancedSearchRequest(BaseModel):
    ...
    user_id: Optional[str] = Field(None, description="User ID for database configuration (optional, will use environment variables if not provided)")
    ...
    table_specific: Optional[bool] = Field(False, description="If True, search only in specified tables")
    tables: Optional[List[str]] = Field(None, description="List of table names to search in when table_specific is True")
...
from data_sources.file_data.app.unstructured.agent.data_upload_v2 import (
...
# --------------------
from data_sources.file_data.app.unstructured.agent.query_answer import DocumentQASystem
...
# --------------------
from data_sources.file_data.app.unstructured.agent.query_answerv2 import EnhancedSearchAnswerPipeline
from data_sources.file_data.app.unstructured.agent.config_loader import get_database_config, DatabaseConfig

# Get API key for enhanced search pipeline
...
async def search_endpoint(request: EnhancedSearchRequest):
    ...
    try:
        ...
        # If user_id is None, get_database_config will use environment variables
        db_config = get_database_config(request.user_id)
        
        # Create pipeline instance with user-specific config
        ...
        results = await enhanced_pipeline.search_and_answer(
            ...
            table_specific=request.table_specific,
        ...
        return {
            ...
            "user_id_used": request.user_id or "environment_variables",
            "database_config": {
                "host": db_config.host,
                "port": db_config.port,
        ...
        logger.error(f"Search failed for query '{request.query}' with user_id '{request.user_id}': {str(e)}")
...
@router.post("/smart_file_system_backend", response_model=Dict, summary="Upload multiple files and automatically route to appropriate processing pipeline")
...
@router.post("/smart_file_system", response_model=Dict, summary="Upload multiple files and automatically route to appropriate processing pipeline")
...
@router.get("/bundle_task_status/{bundle_id}", summary="Get bundle task status with all details")
...
@router.get("/bundle_task_status_all", summary="Get status of all bundle tasks")
...
# @router.delete("/cleanup_old_tasks", summary="Clean up completed tasks")
...
# @router.delete("/cleanup_old_bundles", summary="Clean up completed bundle tasks")
...
# @router.get("/database_status", summary="Get database status")
...
# @router.get("/smart_task_status/{task_id}", response_model=TaskInfo, summary="Get task status for any pipeline")
...
# @router.get("/smart_task_status_all", response_model=List[TaskInfo], summary="Get status of all tasks")
