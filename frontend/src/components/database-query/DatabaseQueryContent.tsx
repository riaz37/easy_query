...
import {
  DatabaseQueryStatsCards,
  DatabaseQueryHeader,
} from "@/components/database-query/components";
...
export function DatabaseQueryContent() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "voice-agent-generate-report",
      handleVoiceAgentGenerateReport as EventListener
    );
    window.addEventListener(
      "voice-agent-show-message",
      handleVoiceAgentShowMessage as EventListener
    );
  ...
  const handleQuerySubmit = useCallback(
    async (query: string) => {
      ...
      const taskId = createQueryTask(
        `${queryMode === "query" ? "Query" : "Report"}: ${query.substring(
          0,
          50
        )}${query.length > 50 ? "..." : ""}`,
        `${
      ...
      <div className="mt-2">
        ...
        <ContentWrapper className="mb-16">
          <div className="space-y-6">
            <div className="px-2 py-2 query-content-gradient relative">
